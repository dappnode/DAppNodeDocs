...
Lido plays a significant role in the Ethereum ecosystem, with more than [9 million ETH](https://lido.fi/ethereum) staked through the platform. Lido stakes these assets on behalf of the users, who receive stETH in return. With the goal of expanding and diversifying their operator set, Lido recently introduced the **Community Staking Module** (CSM), the first Lido module with a **permissionless entry** for Ethereum staking. By first providing an ETH-based bond, anyone can now become a node operator by running validators in the Lido protocol. The module was designed for solo stakers and community stakers, providing them with the unique ability of becoming Ethereum validators with way less than 32 ETH and one of the friendliest UX possible, but may be utilized by anyone, including professional operators.
...
4. **Register as a Lido Node Operator & your validators**:
   ...
   - Navigate to the [Lido Deposit UI](https://csm.testnet.fi/) and complete the deposit process by submitting the `deposit_data.json` file you generated with your keystore(s).
